...
public class GroupServiceImpl implements GroupService {
	...
	private List<Member> toMemberList(List<HashMap> membersFromMeetUp) {
		...
		return membersFromMeetUp.stream()
				.map(memberMap -> Member.builder().name(String.valueOf(memberMap.get("name")))
						.country(String.valueOf(memberMap.get("country"))).city(String.valueOf(memberMap.get("city")))
						.comments(String.valueOf(memberMap.get("bio"))).build())
				.collect(Collectors.toList());
	}

	private List<Group> toGroupList(List<HashMap> techGroups) {
		return techGroups.stream().map(groupMap -> Group.builder().urlname(String.valueOf(groupMap.get("urlname")))
				.name(String.valueOf(groupMap.get("name"))).description(String.valueOf(groupMap.get("description")))
				.link(String.valueOf(groupMap.get("link"))).country(String.valueOf(groupMap.get("country"))).build())
				.collect(Collectors.toList());
	}
